...
def refill_pump():
    ...
    move_mycobot(angles_list['TFA_up'], 20, 3)
    move_mycobot(angles_list['ini'], 20, 3)
    move_mycobot(angles_list['plate_up'], 35, 3)
...
interval_label = tk.Label(root, text="Interval between rounds (minutes):")
interval_label.grid(row=14, column=0, padx=(10, 0), pady=5, sticky="e")

interval_entry = tk.Entry(root)
interval_entry.grid(row=14, column=1, padx=(0, 10), pady=5)

rounds_label = tk.Label(root, text="Number of rounds:")
rounds_label.grid(row=15, column=0, padx=(10, 0), pady=5, sticky="e")

rounds_entry = tk.Entry(root)
rounds_entry.grid(row=15, column=1, padx=(0, 10), pady=5)
# 函数：全选所有选项
def select_all():
...
# 添加按钮来完成液体转移功能
def transfer_liquid():
    selected_volume = volume_selection.get()

    if selected_volume != "Select Volume" and selected_holes:
        interval_minutes = int(interval_entry.get()) if interval_entry.get() else 0
        interval_seconds = interval_minutes * 60
        num_rounds = int(rounds_entry.get()) if rounds_entry.get() else 1

        for i in range(num_rounds):
            is_last_round = (i == num_rounds - 1)  # 检查是否是最后一轮
            for hole in selected_holes:
                dispense_liquid(selected_volume, hole)
                print(f"Transferring {selected_volume} to {hole}")
            empty_pump()
            wait_interval(interval_seconds, is_last_round)  # 等待时间处理改为传递 is_last_round
    else:
        print("Please select at least one hole and a volume.")

# Function to handle waiting period between rounds
def wait_interval(interval_seconds, is_last_round):
    if not is_last_round:
        print(f"Waiting for {interval_seconds // 60} minutes between rounds.")
...
# 添加选择液体体积的下拉菜单
volume_selection = tk.StringVar()
volume_selection.set("Select Volume")

volume_frame = tk.Frame(root)
volume_frame.grid(row=12, column=0, padx=10, pady=10)

tk.Label(volume_frame, text="Select Volume:").grid(row=0, column=0, padx=10)

volumes = list(volume_list.keys())  # 液体体积列表
volume_dropdown = tk.OptionMenu(volume_frame, volume_selection, *volumes)
